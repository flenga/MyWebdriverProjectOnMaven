...
public class CommonFunction extends BaseFunction
{
	...
	public static void ReadExcelFile() throws IOException
	{
		try 
		{
			File src = new File(getData("ExcelFilePath") +"/testData_OnlineStoreOnMaven.xls"); // Excels
	...
	//wait To text in Element to be changed
		public static void waitToTextToChange(WebElement ElementToWait,String text) throws Exception
		{
			try 
			{
				WebDriverWait wait = new WebDriverWait(driver,10);
